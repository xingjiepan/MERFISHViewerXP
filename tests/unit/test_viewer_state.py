...
def test_default_state_activates_all_genes_per_codebook(tmp_path):
    state = ViewerState.load_or_default(
        tmp_path / "settings.json",
        dataset_id="ds",
        cache_path=tmp_path,
        gene_ids_by_codebook={"CB0": [1, 2], "CB1": [3]},
    )
    assert set(state.active_gene_ids_by_codebook["CB0"]) == {1, 2}
    assert set(state.active_gene_ids_by_codebook["CB1"]) == {3}
    assert state.all_active_gene_ids() == {1, 2, 3}


def test_save_and_reload_round_trips_gene_selection(tmp_path):
    settings_path = tmp_path / "settings.json"
    state = ViewerState.load_or_default(
        settings_path, dataset_id="ds", cache_path=tmp_path, gene_ids_by_codebook={"CB0": [1, 2, 3]}
    )
    state.active_gene_ids_by_codebook["CB0"] = [2]
    ...
    assert reloaded.active_gene_ids_by_codebook["CB0"] == [2]
...
def test_corrupt_settings_file_falls_back_to_default(tmp_path):
    ...
    state = ViewerState.load_or_default(
        settings_path, dataset_id="ds", cache_path=tmp_path, gene_ids_by_codebook={"CB0": [1, 2]}
    )
    assert set(state.active_gene_ids_by_codebook["CB0"]) == {1, 2}
...
def test_save_is_atomic_no_partial_file_left(tmp_path):
    settings_path = tmp_path / "settings.json"
    state = ViewerState.load_or_default(
        settings_path, dataset_id="ds", cache_path=tmp_path, gene_ids_by_codebook={"CB0": [1]}
    )
    state.save(settings_path)
...
def test_missing_codebook_gets_no_default_gene_ids_until_app_reconciles(tmp_path):
    ...
    settings_path = tmp_path / "settings.json"
    settings_path.write_text(json.dumps({"active_gene_ids_by_codebook": {"CB0": [1]}}))
    state = ViewerState.load_or_default(
        settings_path, dataset_id="ds", cache_path=tmp_path, gene_ids_by_codebook={"CB0": [1, 2], "CB1": [3]}
    )
    assert state.active_gene_ids_by_codebook == {"CB0": [1]}
